...
CDarkSendRelay::CDarkSendRelay(CTxIn& vinFivegnodeIn, vector<unsigned char>& vchSigIn, int nBlockHeightIn, int nRelayTypeIn, CTxIn& in2, CTxOut& out2)
{
    vinFivegnode = vinFivegnodeIn;
    vchSig = vchSigIn;
...
std::string CDarkSendRelay::ToString()
{
    ...
    info << "vin: " << vinFivegnode.ToString() <<
...
{
    CFivegnode* pmn = mnodeman.GetFivegnodeByRank(nRank, nBlockHeight, MIN_PRIVATESEND_PEER_PROTO_VERSION);
