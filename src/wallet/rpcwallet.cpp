...
UniValue getaccountaddress(const UniValue& params, bool fHelp)
{
    ...
    if (fHelp || params.size() != 1)
        throw runtime_error(
            ...
            "\nDEPRECATED. Returns the current Fiveg address for receiving payments to this account.\n"
            "\nArguments:\n"
...
UniValue sendtoaddress(const UniValue& params, bool fHelp)
{
    ...
    if (fHelp || params.size() < 2 || params.size() > 5)
        throw runtime_error(
            "sendtoaddress \"fivegaddress\" amount ( \"comment\" \"comment-to\" subtractfeefromamount )\n"
...
UniValue sendfrom(const UniValue& params, bool fHelp)
{
    ...
    if (fHelp || params.size() < 3 || params.size() > 6)
        throw runtime_error(
            ...
            "\nDEPRECATED (use sendtoaddress). Sent an amount from an account to a Fiveg address."
            + HelpRequiringPassphrase() + "\n"
            "\nArguments:\n"
...
UniValue sendmany(const UniValue& params, bool fHelp)
{
    ...
    if (fHelp || params.size() < 2 || params.size() > 5)
        throw runtime_error(
            ...
            "    {\n"
            "      \"address\":amount   (numeric or string) The Fiveg address is the key, the numeric amount (can be string) in " + CURRENCY_UNIT + " is the value\n"
            "      ,...\n"
            "    }\n"
            ...
            if (fAllAccounts || (account == strAccount))
            {
                ...
                if (wtx.IsCoinBase())
                {
                    ...
                    if(addr.ToString() == payeeAddr.ToString()){
                        entry.push_back(Pair("category", "fivegnode"));
                    }
...
UniValue encryptwallet(const UniValue& params, bool fHelp)
{
    ...
    return "wallet encrypted; Fiveg server stopping, restart to run with encrypted wallet.";
}
...
UniValue fundrawtransaction(const UniValue& params, bool fHelp)
{
    ...
    if (fHelp || params.size() < 1 || params.size() > 2)
        throw runtime_error(
                            ...
                            "   {\n"
                            "     \"changeAddress\"     (string, optional, default pool address) The Fiveg address to receive the change\n"
    ...
    if (params.size() > 1) {
      if (params[1].type() == UniValue::VBOOL) {
        ...
        if (options.exists("changeAddress")) {
            ...
            if (!address.IsValid())
                throw JSONRPCError(RPC_INVALID_PARAMETER, "changeAddress must be a valid Fiveg address");
...
UniValue spendzerocoin(const UniValue& params, bool fHelp) {
    ...
    if (fHelp || params.size() < 1 || params.size() > 2)
        throw runtime_error(
                "spendzerocoin <amount>(1,10,25,50,100) (\"fivegaddress\")\n"
                + HelpRequiringPassphrase() +
				"\nArguments:\n"
				...
				"2. \"fivegaddress\"  (string, optional) The Fiveg address to send to third party.\n"
				"\nExamples:\n"
...
UniValue spendmanyzerocoin(const UniValue& params, bool fHelp) {
    ...
    for(size_t i=0; i<inputs.size();i++) {
        ...
        switch(value){
            ...
            default:
                throw runtime_error(
                    "spendmanyzerocoin <amount>(1,10,25,50,100) (\"fivegaddress\")\n");
        }
...
UniValue spendmany(const UniValue& params, bool fHelp) {
    ...
    for (const auto& strAddr : keys) {
        ...
        if (!address.IsValid())
            throw JSONRPCError(RPC_INVALID_ADDRESS_OR_KEY, "Invalid Fiveg address: " + strAddr);
