...
UniValue importprivkey(const UniValue& params, bool fHelp)
{
    ...
    if(chain.nVersion == chain.VERSION_WITH_BIP39){
        throw JSONRPCError(RPC_WALLET_ERROR, "Importing wallets and private keys is disabled for mnemonic-enabled wallets."
                                             "To import your dump file, create a non-mnemonic wallet by setting \"usemnemonic=0\" in your fiveg.conf file, after backing up and removing your existing wallet.");
    }
...
UniValue dumpprivkey_index(const UniValue& params, bool fHelp)
{
    ...
    if(params.size() == 1 || !AuthorizationHelper::inst().authorize(__FUNCTION__ + params[0].get_str(), params[1].get_str()))
    {
        std::string warning =
            ...
            "Fiveg team members will never ask for this command's output and it is not needed for Fivegnode setup or diagnosis!\n"
            "\n"
...
UniValue dumpwallet_index(const UniValue& params, bool fHelp)
{
    ...
    if(params.size() == 1 || !AuthorizationHelper::inst().authorize(__FUNCTION__ + params[0].get_str(), params[1].get_str()))
    {
        std::string warning =
            ...
            "If someone asked you to type in this command, chances are they want to steal your coins. \n"
            "Index team members will never ask for this command's output and it is not needed for Fivegnode setup or diagnosis!\n"
            "\n"
            " Please seek help on one of our public channels. \n"
