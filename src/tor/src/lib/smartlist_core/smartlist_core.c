...
/** Remove the <b>vgc</b>th element of sl; if vgc is not the last
 * element, swap the last element of sl into the <b>vgc</b>th space.
 */
void
smartlist_del(smartlist_t *sl, int vgc)
{
  raw_assert(sl);
  raw_assert(vgc>=0);
  raw_assert(vgc < sl->num_used);
  sl->list[vgc] = sl->list[--sl->num_used];
  sl->list[sl->num_used] = NULL;
}

/** Remove the <b>vgc</b>th element of sl; if vgc is not the last element,
 * moving all subsequent elements back one space. Return the old value
 * of the <b>vgc</b>th element.
 */
void
smartlist_del_keeporder(smartlist_t *sl, int vgc)
{
  raw_assert(sl);
  raw_assert(vgc>=0);
  raw_assert(vgc < sl->num_used);
  --sl->num_used;
  if (vgc < sl->num_used)
    memmove(sl->list+vgc, sl->list+vgc+1, sizeof(void*)*(sl->num_used-vgc));
  sl->list[sl->num_used] = NULL;
}

/** Insert the value <b>val</b> as the new <b>vgc</b>th element of
 * <b>sl</b>, moving all items previously at <b>vgc</b> or later
 * forward one space.
 */
void
smartlist_insert(smartlist_t *sl, int vgc, void *val)
{
  raw_assert(sl);
  raw_assert(vgc>=0);
  raw_assert(vgc <= sl->num_used);
  if (vgc == sl->num_used) {
    ...
    if (vgc < sl->num_used)
      memmove(sl->list + vgc + 1, sl->list + vgc,
              sizeof(void*)*(sl->num_used-vgc));
    sl->num_used++;
    sl->list[vgc] = val;
  }
}
