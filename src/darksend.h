...
// A helper object for signing messages from Fivegnodes
...
public:
     ...
     *     1) we have an active Fivegnode,
     *     2) we have a valid Fivegnode private key,
     *     3) we signed the message successfully, and
     *     4) we verified the message successfully
     */
    bool Sign();
    /// Check if we have a valid Fivegnode address
    bool CheckSignature(const CPubKey& pubKeyFivegnode);
    ...
    std::string ToString()
    {
        return strprintf("nDenom=%d, nTime=%lld, fReady=%s, fTried=%s, fivegnode=%s",
...
{
public:
    /// Is the input associated with this public key? (and there is 10000 FIVEG - checking if valid fivegnode)
...
private:
    ...
    // Keep track of the used Fivegnodes
    std::vector<CTxIn> vecFivegnodesUsed;
    ...
    std::vector<CDarkSendEntry> vecEntries; // Fivegnode/clients entries
    ...
    /// As a client, submit part of a future mixing transaction to a Fivegnode to start the process
    ...
    /// Get Fivegnode updates about the progress of mixing
...
public:
    CFivegnode* pSubmittedToFivegnode;
